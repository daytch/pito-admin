...
const Edit = ({ data = JSON.parse(localStorage.getItem('data')), openLoading, closeLoading }) => {
    ...
    useEffect(() => {
        livestream.getCategory().then((res) => {
            let catArr = [];
            let ListCategory = res.data.map((i) => {
        ...
        if (ids.length < 1) {
            livestream.getCategory().then((res) => {
                ...
                res.data.map((i) => {
                    ...
                    if (i.text == data.item.categories[1]) {
                        ids.push(i.id)
                    }
                    if (i.text == data.item.categories[2]) {
                        ids.push(i.id)
                    }
                })
            ...
            <div className="w-full xl:w-4/5">
                <form>
                    ...
                    <div className="flex mt-6">
                        <Link to={`/livestream`} className="border border-gray-300 text-red-600 rounded-md text-sm px-6 py-2 mr-4">Cancel</Link>
